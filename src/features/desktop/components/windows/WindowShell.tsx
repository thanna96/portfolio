...
export function WindowShell({
    ...
    return (
        ...
        >
            ...
            <div className="flex h-6 items-center gap-2 border border-black/40 bg-[#C0C0C0] px-2 text-xs font-black">
                {menuContent ??
                    defaultMenu.map((item) => (
                        <span key={item} className="border-r border-black/40 pr-2 last:border-none">
              {item}
            </span>
                    ))}
            </div>
